 ...
 *  - plymouth (which uses dumb-buffers like this example; very easy to
 *understand)
 ...
 *  - xserver (very hard to understand as it is split across many
 *files/projects)
 *
...
int modeset_get_fb_width() {
  struct modeset_buf *buf;
  struct modeset_dev *dev = modeset_list;
  buf = &dev->bufs[dev->front_buf ^ 1];
  return buf->width;
}
int modeset_get_fb_height() {
  struct modeset_buf *buf;
  struct modeset_dev *dev = modeset_list;
  buf = &dev->bufs[dev->front_buf ^ 1];
  return buf->height;
